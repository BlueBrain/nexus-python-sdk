...
def http_get(url, params=None, use_base=False):
...
def http_post(url, body=None, data_type='default', params=None):
...
def http_put(url, body=None, data_type='default', params=None, use_base=False):
    header = prepare_header()
    full_url = (storage.get('environment') if use_base else '') + url
    body_data = prepare_body(body, data_type)
...
def http_patch(url, body=None, data_type='default', params=None, use_base=False):
    header = prepare_header()
    full_url = (storage.get('environment') if use_base else '') + url
    body_data = prepare_body(body, data_type)
...
def http_delete(url, body=None, data_type='default', params=None, use_base=False):
    header = prepare_header()
    full_url = (storage.get('environment') if use_base else '') + url
    body_data = prepare_body(body, data_type)
